...
parser.add_argument("--regime", help="Choose regime (Frame or Augmentation_parameter):",default = Regime.FRAME)
...
class RegimeProcessor(InputProcessor):
    ...
    @classmethod
    def process(clf,input_argument):

        if input_argument == Regime.FRAME:
            clf.__frame_videos()
        
        elif input_argument == Regime.SPLIT:
            not_video_frames = NotVideoFrames()
            not_video_frames.split()

        elif input_argument == Regime.BALANCE:
