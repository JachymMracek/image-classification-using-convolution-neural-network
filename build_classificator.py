...
parser.add_argument("--train_mode", help="Choose what type of training do you want:",default = TrainModes.FULL_FIT)
...
class TypeModel(Parameter):
    ...
    BEST_TYPE_MODEL = models.maxvit_t # set it handly, else None
    ...
    def searching(self,count_classes):
        ...
        for type_model in self.values:
            ...
            model.fit(Copies.DEFAULT_COPIES,self.EPOCHS)
...
class TypeOptimizer(Parameter):
    ...
    def searching(self,count_classes):
        ...
        for type_optimizer in self.values:
            print(self.accuracies)
            model = Model(TypeModel.BEST_TYPE_MODEL(weights = "DEFAULT"),type_optimizer,count_classes)
            model.fit(Copies.DEFAULT_COPIES,self.EPOCHS)
            type_optimizer_name = type_optimizer[0].__name__
            self.accuracies[type_optimizer_name] = model.best_validation_accuracy

class lrMomentum(Parameter):

    BEST_LR_MOMENTUM = (0.01,0.4) # set it handly, else None
    ...
    def get_best_lr_momentum_optimizer(self):
        optimizer_type,default_parameters = TypeOptimizer.BEST_TYPE_OPTIMIZER
        best_lr,best_momentum = self.BEST_LR_MOMENTUM

        parameters = {**default_parameters,"lr":best_lr,"momentum":best_momentum}

        return (optimizer_type,parameters)
    ...
    def searching(self,count_classes):
        ...
        for lr_momentum in self.values:
            ...
            parameters = {**default_parameters,"lr":lr_momentum[0],"momentum":lr_momentum[1]}
            optimizer_parameters = (optimizer_type,parameters)
            model = Model(TypeModel.BEST_TYPE_MODEL(weights = "DEFAULT"),optimizer_parameters,count_classes)
            model.fit(Copies.DEFAULT_COPIES,self.EPOCHS)
...
class WeightDecay(Parameter):
    ...
    @classmethod
    def get_best_lr_momentum_weight_optimizer(clf):
        optimmizer_type,parameters = lrMomentum.get_best_lr_momentum_optimizer()

        parameters = {**parameters,"weight_decay" : clf.BEST_WEIGHT_DECAY_VALUES}

        return (optimmizer_type,parameters)
    
    # searching for best weight_decay parameter
    @override
    def searching(self, count_classes):

        optimizer_type,parameters = lrMomentum.get_best_lr_momentum_optimizer()
...
class Nesterov(Parameter):
    ...
    @classmethod
    def get_best_lr_momentum_weight_nesterov_optimizer(clf):
        optimizer_type,parameters = WeightDecay.get_best_lr_momentum_weight_optimizer()

        parameters = {**parameters,"nesterov":clf.BEST_NESTEROV_VALUE}
    ...
    def searching(self,count_classes):
        ...
        for nesterov_value in self.values:
            ...
            updated_parameters = {**parameters,"nesterov":nesterov_value}
            model = Model(TypeModel.BEST_TYPE_MODEL(weights="DEFAULT"),(optimizer_type,updated_parameters),count_classes)
            model.fit(Copies.DEFAULT_COPIES,self.EPOCHS)
...
class Copies(Parameter):
    ...
    def searching(self,count_classes):
        ...
        optimizer_type,parameters = Nesterov.get_best_lr_momentum_weight_nesterov_optimizer()

        for copies in self.values:
            print(self.accuracies)
            model = Model(TypeModel.BEST_TYPE_MODEL(weights="DEFAULT"),(optimizer_type,parameters),count_classes)
            model.fit(copies,self.EPOCHS)
...
class Model(nn.Module):
    ...
    def __init__(self,model,optimizer,count_classes):
        ...
        if isinstance(model, (VGG, AlexNet)):                                                                        ##                                                                  
            ...
            model.classifier[-1] = nn.Linear(in_features, count_classes).to(self.device)                             ##
                                                                                                                     ## Tady generovaná část končí. Zpetám se vedoucího, jestli ok.
    ...
    # returns top predicted classes and its probabilities
    @staticmethod
    def classify_img(img,model,TOP_CLASSES_NUM = 5):
            ...
            with torch.no_grad():
                ...
                top_probabilities, top_classes = probabilities.topk(TOP_CLASSES_NUM, dim=1)

                top_probabilities = top_probabilities.squeeze(0)
                top_classes = top_classes.squeeze(0)


                for i in range(TOP_CLASSES_NUM):

                    top_i_class_probability = (top_classes[i].item(),round(top_probabilities[i].item()*100,2))
                    top_classes_probabilities.append(top_i_class_probability)

            return top_classes_probabilities
...
class InputProcessorTraining(InputProcessor):
    ...
    def process(self, input_argument,epochs_count = 20):
        ...
        match input_argument:
            ...
            case TrainModes.COPIES:
                Copies().searching(self.count_classes)
            ...
            case TrainModes.FULL_FIT:
                ...
                model = Model(TypeModel.BEST_TYPE_MODEL(weights = "DEFAULT"),optimizer,self.count_classes)
